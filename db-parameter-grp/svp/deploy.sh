...
terraform workspace select parameter-"${application}"-"${environment}" || terraform workspace new parameter-"${application}"-"${environment}"
terraform ${action} -var "env=${environment}" -var-file=${environment}.tfvars -input=false
